...
input_topic_Phishing = app.topic("phishing-complex", value_deserializer="json")
...
almaPipeline = Pipeline(model = almaPipeline_pipeline, model_name ='ALMAClassifier'  , metrics_list = almaPipeline_metrics , name = "almaPipeline",y="data_is_phishing",output_topic="almaPipeline")
    ...
    def update_stats_LinearRegressionPipeline(n):
        ...
        if traces:
            fig = go.Figure(
                    data=traces, 
                    layout= go.Layout(
                        title='LinearRegressionPipeline Statistics'
                    )
            )
            return fig    
