...
Chebyshev = imblearn.ChebyshevOverSampler(
    regressor = amr
)
mae = metrics.MAE(
)
...
ChebyshevPipeline_pipeline =Chebyshev
ChebyshevPipeline_metrics = [mae]


ChebyshevPipeline = Pipeline(model = ChebyshevPipeline_pipeline, model_name ='ChebyshevOverSampler'  , metrics_list = ChebyshevPipeline_metrics , name = "ChebyshevPipeline",y="five_thirty_eight",output_topic="ChebyshevPipeline")



# Output topics initialization

output_topic_ChebyshevPipeline = app.topic(ChebyshevPipeline.output_topic, value_deserializer="json")
...
sdf_ChebyshevPipeline = sdf_TrumpApproval.apply(ChebyshevPipeline.train_and_predict).to_topic(output_topic_ChebyshevPipeline)
...
def run_dash():
    ...
    dash_app.layout = html.Div([
        ...
        html.Div(
            children=[
                ...
                dcc.Graph(
                    id='live-stats-ChebyshevPipeline',
    ...
    def update_graph(n):
        ...
        ChebyshevPipeline.add_metrics_traces(fig = fig , row = 1, col = 1 ) 
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-ChebyshevPipeline', 
    ...
    def update_stats_ChebyshevPipeline(n):
        
        traces = []  
        
        ChebyshevPipeline.add_stats_traces(traces) 
        ...
        if traces:
            fig = go.Figure(
                    data=traces, 
                    layout= go.Layout(
                        title='ChebyshevPipeline Statistics'
                    )
            )
            return fig    
