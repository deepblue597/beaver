...
input_topic_clusterd = app.topic("cluster", value_deserializer="json")

# Create Streaming DataFrames connected to the input Kafka topics

sdf_clusterd = app.dataframe(topic=input_topic_clusterd)
...
clusterPipeline_pipeline =cluster

clusterPipeline = Pipeline(model = clusterPipeline_pipeline, model_name ='CluStream' , name = "clusterPipeline",output_topic="clusterPipeline")



# Output topics initialization

output_topic_clusterPipeline = app.topic(clusterPipeline.output_topic, value_deserializer="json")
...
sdf_clusterPipeline = sdf_clusterd.apply(clusterPipeline.train_and_predict).to_topic(output_topic_clusterPipeline)
...
def run_dash():
    ...
    def update_graph(n):
        ...
        clusterPipeline.add_metrics_traces(fig = fig , row = 1, col = 1 ) 
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-clusterPipeline', 
    ...
    def update_stats_clusterPipeline(n):
        
        traces = []  
        
        clusterPipeline.add_stats_traces(traces) 
        ...
        if traces:
            ...
            fig = go.Figure(
                    data=traces, 
                    layout= go.Layout(
                        title='clusterPipeline Statistics'
                    )
            )
            return fig    
