...
KNNClassifierPipeline_metrics = [accuracy,recall,f1]
...
ARFClassifierPipeline_pipeline = preprocessor_Heart_Failure_Prediction |arf
ARFClassifierPipeline_metrics = [accuracy,recall,f1]


ARFClassifierPipeline = Pipeline(model = ARFClassifierPipeline_pipeline, model_name ='ARFClassifier'  , metrics_list = ARFClassifierPipeline_metrics , name = "ARFClassifierPipeline",y="HeartDisease",output_topic="ARFClassifierPipeline")

logisticPipeline_pipeline = preprocessor_Heart_Failure_Prediction |logistic
logisticPipeline_metrics = [accuracy,recall,f1]
...
output_topic_ARFClassifierPipeline = app.topic(ARFClassifierPipeline.output_topic, value_deserializer="json")
...
sdf_ARFClassifierPipeline = sdf_Heart_Failure_Prediction.apply(ARFClassifierPipeline.train_and_predict).to_topic(output_topic_ARFClassifierPipeline)
...
def run_dash():
    ...
    dash_app.layout = html.Div([
        ...
        html.Div(
            children=[
                ...
                dcc.Graph(
                    id='live-stats-ARFClassifierPipeline',
                    #style={'margin': 'auto', 'display': 'block', 'width':'70%'}
                ),
                dcc.Graph(
    ...
    def update_graph(n):
        ...
        ARFClassifierPipeline.add_metrics_traces(fig = fig , row = 2, col = 1 ) 
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-ARFClassifierPipeline', 
            component_property='figure'
        ), 
        Input(
            component_id='interval', 
    ...
    def update_stats_ARFClassifierPipeline(n):
        
        traces = []  
        
        ARFClassifierPipeline.add_stats_traces(traces) 
        ...
        if traces:
            fig = go.Figure(
                    data=traces, 
                    layout= go.Layout(
                        title='ARFClassifierPipeline Statistics'
                    )
            )
            return fig    
