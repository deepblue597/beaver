...
amfPipeline_pipeline =amf
amfPipeline_metrics = [mae]


amfPipeline = Pipeline(model = amfPipeline_pipeline, model_name ='AMFRegressor'  , metrics_list = amfPipeline_metrics , name = "amfPipeline",y="five_thirty_eight",output_topic="amfPipeline")



# Output topics initialization

output_topic_amfPipeline = app.topic(amfPipeline.output_topic, value_deserializer="json")
...
sdf_amfPipeline = sdf_TrumpApproval.apply(amfPipeline.train_and_predict).to_topic(output_topic_amfPipeline)
...
def run_dash():
    ...
    def update_graph(n):
        ...
        amfPipeline.add_metrics_traces(fig = fig , row = 1, col = 1 ) 
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-amfPipeline', 
    ...
    def update_stats_amfPipeline(n):
        
        traces = []  
        
        amfPipeline.add_stats_traces(traces) 
