...
knnPipeline_pipeline =knn
knnPipeline_metrics = [rmse]


knnPipeline = Pipeline(model = knnPipeline_pipeline, model_name ='KNNRegressor'  , metrics_list = knnPipeline_metrics , name = "knnPipeline",y="five_thirty_eight",output_topic="knnPipeline")



# Output topics initialization

output_topic_knnPipeline = app.topic(knnPipeline.output_topic, value_deserializer="json")
...
sdf_knnPipeline = sdf_TrumpApproval.apply(knnPipeline.train_and_predict).to_topic(output_topic_knnPipeline)
...
def run_dash():
    ...
    def update_graph(n):
        ...
        knnPipeline.add_metrics_traces(fig = fig , row = 1, col = 1 ) 
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-knnPipeline', 
    ...
    def update_stats_knnPipeline(n):
        
        traces = []  
        
        knnPipeline.add_stats_traces(traces) 
