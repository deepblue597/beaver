...
def run_dash():
    ...
    dash_app.layout = html.Div([
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Div([
                        html.H4("LinearRegressionPipeline", 
                               style={'margin': '0', 'color': '#2E86AB'}),
                        html.P("", 
                              style={'margin': '5px 0', 'color': '#666', 'fontSize': '0.9em'}),
                        html.Div(id='status-LinearRegressionPipeline', 
                                children="🟢 Active",
                                style={'fontWeight': 'bold', 'color': '#28a745'})
                    ], style={
                        'backgroundColor': '#ffffff',
        ...
        html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.H4("almaPipeline Detailed Stats", 
                           style={'color': '#2E86AB', 'textAlign': 'center', 'marginBottom': '15px'}),
                    dcc.Graph(
                        id='live-stats-almaPipeline',
                ...
                html.Div([
                    html.H4("LinearRegressionPipeline Detailed Stats", 
                           style={'color': '#2E86AB', 'textAlign': 'center', 'marginBottom': '15px'}),
                    dcc.Graph(
                        id='live-stats-LinearRegressionPipeline',
                        style={'height': '400px', 'border': '1px solid #e9ecef', 'borderRadius': '8px'}
                    )
                ], style={
                    'backgroundColor': '#ffffff',
    ...
    def update_graph(n):
        fig = make_subplots(
            ...
            subplot_titles=["almaPipeline ()", "LinearRegressionPipeline ()"]
        )
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-almaPipeline', 
    ...
    def update_stats_almaPipeline(n):
        traces = []  
        ...
        if traces:
            fig = go.Figure(
                ...
                layout=go.Layout(
                    title={
                        'text': 'almaPipeline Detailed Statistics',
                        'x': 0.5,
                        'xanchor': 'center',
    ...
    @dash_app.callback(
        Output(
            component_id='live-stats-LinearRegressionPipeline', 
            component_property='figure'
        ), 
        Input(
            component_id='interval', 
    ...
    def update_stats_LinearRegressionPipeline(n):
        traces = []  
        ...
        if traces:
            fig = go.Figure(
                ...
                layout=go.Layout(
                    title={
                        'text': 'LinearRegressionPipeline Detailed Statistics',
                        'x': 0.5,
                        'xanchor': 'center',
                        'font': {'size': 16, 'color': '#2E86AB'}
                    },
    ...
    @dash_app.callback(
        Output('status-almaPipeline', 'children'),
        Input('interval', 'n_intervals')
    )
    def update_status_almaPipeline(n):
        # You can add logic here to check actual pipeline status
        # For now, we'll show active status
        return "🟢 Active"
    
    @dash_app.callback(
        Output('status-LinearRegressionPipeline', 'children'),
        Input('interval', 'n_intervals')
    )
    def update_status_LinearRegressionPipeline(n):
        # You can add logic here to check actual pipeline status
        # For now, we'll show active status
        return "🟢 Active"
