...
output_topic = app.topic("SoftmaxRegression-results",
...
def train_and_predict(event):
    ...
    X = {key: value for key, value in event.items() if key != "category"}  
    ...
    print(metric)
    MacroF1.append(metric.get()) 
    
    


    with open('SoftmaxRegression.pkl', 'wb') as model_file:
...
plt.ylabel('MacroF1')
plt.title('MacroF1 over Training Iterations')
plt.show()
